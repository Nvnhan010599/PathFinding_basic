...
class A_star_Algorithm:
    ...
    def calculus_Dis(self,xk,yk):
        ...
        for i in range(len(listIndex_check)):
            ...
            if(self.U[3][point] != 1):
                ...
                x_o = lbx + ix*resolution
                y_o = lby + iy*resolution
                g = math.sqrt((xk - x_o)**2 + (yk-y_o)**2)
                h = math.sqrt((self.target_point[0] - x_o)**2 + (self.target_point[1]-y_o)**2)
                f = g + h
                
                if(f + S[step][1] < self.U[1][point] + h):
                    self.U[1][point] = round(g + S[step][1],2)
                    self.U[4][point] = round(f + S[step][1],2)
                    self.U[2][point] = k      
    ...
    def result(self):
        ...
        for point in range(len(path)):
            ...
            x_path = self.lbx + ix_path*self.resolution
            y_path = self.lby + iy_path*self.resolution
            Target.append([x_path,y_path])
